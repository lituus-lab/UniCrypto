...
task coverage, "LCOV + HTML coverage report for the Nim sources (needs lcov)":
  ...
  exec "nim c --app:staticlib -d:noAutoInit --noMain --mm:arc -d:release --debugger:native" &
       ...
       " -o:build/libUniCrypto_cov.a src/UniCrypto/c_api.nim"
  ...
  exec makeExe & " -C tests/c BIN=test_capi_cov" &
       " LIB=../../build/libUniCrypto_cov.a EXTRA_CFLAGS=--coverage"
  ...
  exec "lcov --capture --no-function-coverage --directory " & capiCache &
       " --base-directory ." &
       " --include \"*/src/UniCrypto/*\" --output-file build/capi.info --quiet" &
